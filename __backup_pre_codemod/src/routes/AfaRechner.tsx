    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">GebÃ¤udeanteil, Modernisierungen, Sonder-AfA â€“ mit Pro-rata & einfacher Steuerwirkung.</p>
        </div>
      ...
      <div className="rounded-2xl bg-white border shadow-sm p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <NumberField label="Horizont (Jahre)" value={input.horizonYears} onChange={(v) => setInput((s) => ({ ...s, horizonYears: clamp(Math.round(v), 1, 40) }))} help="Wie viele Prognosejahre mÃ¶chtest du sehen?" />
        </div>
          ...
          <div className="md:col-span-2 rounded-xl border p-3">
            ...
            {input.proratOn && (
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <div className="text-xs text-gray-500 self-end">AfA in Y1 = Jahres-AfA Ã— {Math.round(proratY1Main * 12)}/12</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-white border shadow-sm p-4 space-y-3">
        ...
        {input.taxOn && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <PercentField label="Grenzsteuersatz (%)" value={input.marginalTaxPct * 100} onChange={(p) => setInput((s) => ({ ...s, marginalTaxPct: clamp(p, 0, 100) / 100 }))} step={0.5} help="PersÃ¶nlicher Steuersatz am Rand (vereinfachte Annahme)." />
      ...
      <p className="text-xs text-gray-500">Hinweis: Vereinfachtes Modell. Keine Steuer-/Rechtsberatung. Detailregeln (AfA-SÃ¤tze, Umqualifizierung Erhaltungs-/HK etc.) sind bewusst vereinfacht.</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl bg-white border shadow-sm p-4 space-y-3">
      ...
      <div className="space-y-3">
          ...
          <div key={m.id} className="border rounded-xl p-3">
            <div className="grid grid-cols-1 md:grid-cols-8 gap-3 items-end">
              ...
              <div className={`border rounded-lg p-2 ${m.capitalize ? "" : "opacity-50 pointer-events-none"}`}>
                ...
                <div className="text-[11px] text-gray-500 mt-1">AfA in Y1 = Jahres-AfA Ã— {(m.proratOn ? Math.round(monthsFactor(m.startMonat ?? 1) * 12) : 12)}/12</div>
              </div>
        ...
        {input.modernisierungen.length === 0 && <p className="text-xs text-gray-500">Keine Positionen hinzugefÃ¼gt.</p>}
      </div>
    </div>
  );
}
          ...
          <div key={p.id} className="border rounded-xl p-3">
            <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
              ...
              <div className={`border rounded-lg p-2 ${p ? "" : "opacity-50 pointer-events-none"}`}>
                ...
                <div className="text-[11px] text-gray-500 mt-1">AfA in Y1 = Jahres-AfA Ã— {(p.proratOn ? Math.round(monthsFactor(p.startMonat ?? 1) * 12) : 12)}/12</div>
              </div>
