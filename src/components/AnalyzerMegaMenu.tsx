      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {[["BASIS-ANALYZER", basisModules], ["PRO-ANALYZER", proModules]].map(([label, mods]) => (
          <div key={label as string}>
            <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: TEXT_DIM, marginBottom: 8 }}>{label as string}</div>
                ...
                <NavLink key={m.key} to={locked ? "/upgrade" : m.href} onClick={onNavigate}
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: TEXT }}>
                      {m.title}
                      {m.requiredPlan === "pro" && <span style={{ fontSize: 12, padding: "1px 6px", borderRadius: 20, background: "rgba(252,220,69,0.12)", color: YELLOW, border: "1px solid rgba(252,220,69,0.25)", fontWeight: 600 }}>PRO</span>}
                    </div>
                    <div style={{ fontSize: 13, color: TEXT_DIM, marginTop: 2 }}>{m.description}</div>
                  </div>
                </NavLink>
              );
            })}
      ...
      {open && (
        <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 520, background: "#111318", border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20, boxShadow: "0 12px 40px rgba(0,0,0,0.5)", zIndex: 50 }}>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: TEXT_DIM }}>Immo-Analyzer</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.8)", marginTop: 2 }}>Wähle ein Tool, um dein Objekt zu prüfen.</div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            {/* Basis */}
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: TEXT_DIM, marginBottom: 10 }}>Basis-Analyzer</div>
                ...
                {basisModules.map((m) => (
                  <NavLink key={m.key} to={m.href} onClick={() => { setOpen(false); onNavigate?.(); }}
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: TEXT }}>
                        {m.title}
                        {m.requiredPlan === "basis" && <span style={{ fontSize: 12, padding: "1px 6px", borderRadius: 20, background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.85)", border: "1px solid rgba(255,255,255,0.1)", fontWeight: 600 }}>BASIS</span>}
                      </div>
                      <div style={{ fontSize: 13, color: TEXT_DIM, marginTop: 1, lineHeight: 1.4 }}>{m.description}</div>
                    </div>
                  </NavLink>
                ))}
              </div>
            ...
            {/* PRO */}
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: TEXT_DIM, marginBottom: 10 }}>PRO-Analyzer</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {proModules.map((m) => {
                  ...
                  return (
                    <NavLink key={m.key} to={locked ? "/upgrade" : m.href} onClick={() => { setOpen(false); onNavigate?.(); }}
                      ...
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: TEXT }}>
                          {m.title}
                          <span style={{ fontSize: 12, padding: "1px 6px", borderRadius: 20, background: "rgba(252,220,69,0.12)", color: YELLOW, border: "1px solid rgba(252,220,69,0.25)", fontWeight: 600 }}>PRO</span>
