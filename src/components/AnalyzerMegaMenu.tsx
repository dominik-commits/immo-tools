...
export default function AnalyzerMegaMenu({ modules, variant = "desktop", onNavigate }: Props) {
  if (variant === "mobile") {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: TEXT_DIM, marginBottom: 8 }}>Analyzer</div>
        {modules.map((m) => (
          <NavLink key={m.key} to={m.href} onClick={onNavigate}
  ...
  return (
    <div ref={containerRef} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 320, background: "#111318", border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20, boxShadow: "0 12px 40px rgba(0,0,0,0.5)", zIndex: 50 }}>
