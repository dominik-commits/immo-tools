...
import type { MfhProInput, MfhProResult } from "../core/mfhCalc";

const DEBOUNCE_MS = 500;

export function useMfhProAnalysis(input: MfhProInput, plan: UserPlan) {
  const { getToken } = useAuth();
  const [data, setData] = useState<MfhProResult | null>(null);
