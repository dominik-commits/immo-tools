...
import type { EfhProInput, EfhProResult } from "../core/efhCalc";

const DEBOUNCE_MS = 500;

export function useEfhProAnalysis(input: EfhProInput, plan: UserPlan) {
  const { getToken } = useAuth();
  const [data, setData] = useState<EfhProResult | null>(null);
