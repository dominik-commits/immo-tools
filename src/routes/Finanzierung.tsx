...
// v1.2 – verständlichere UI, Farben & spielerische Elemente
...
type Input = {
  ...
  zinsbindungJahre: number; // nur Info-KPI, Rate bleibt Annuität aus (Zins+Tilgung)

  // Planung
  laufzeitJahre: number;    // Gesamthorizont
};

const DRAFT_KEY = "finance.tool.v1.2";
...
function applyPreset(kind: "80" | "90" | "100") {
  ...
  // (Kleiner Trick: Wir nutzen die Funktion als Factory)
  return (setInput: React.Dispatch<React.SetStateAction<Input>>, s: Input) => {
    if (kind === "80") {
      ...
      const ek = 0;
      setInput({ ...s, eigenkapital: ek, tilgungStartPct: 0.02, maklerPct: s.maklerPct }); // unverändert
    }
  };
}
...
// Rewire Chip onClick leicht: wir wollen state innen:
...
// Wir nutzen in der Komponente die einfachen Chips:
