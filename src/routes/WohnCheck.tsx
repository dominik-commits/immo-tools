...
/** Wohn-Check – strikt einspaltig + farbige Projektion
 * - Einspaltiges Layout in allen Bereichen
 * - Profit-Spielplatz (wirkt optional global)
 * - Wert vs. Kaufpreis mit Gradients & Delta-Badge
 * - Excel-ähnliche Auswertung + 10J-Projektion (Linien farblich unterscheidbar)
 */
...
export function WohnCheck() {
  ...
  // Ansicht wählen (alles downstream nutzt „view“)
  ...
  // NK-Beträge (Anzeige auf Basis viewInput)
  ...
  // --- Excel-ähnliche Auswertung (auf Basis viewInput) ---
  ...
  // --- 10-J-Projektion (farbige Linien) ---
  const projYears = 10;
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold tracking-tight">Wohn-Check</h2>
              <p className="text-gray-600 text-sm">Einfach, visuell, spielerisch – mit Live-Score & Break-even.</p>
            </div>
          </div>
        ...
        {/* Eingaben (einspaltig) */}
        <div className="rounded-2xl p-[1px] bg-gradient-to-br from-sky-200/60 via-fuchsia-200/50 to-emerald-200/60">
          <div className="rounded-2xl bg-white p-5 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              {/* Kaufnebenkosten */}
              <fieldset className="border rounded-xl p-3 bg-gray-50">
                <legend className="text-sm font-medium">Kaufnebenkosten (transparent)</legend>
              ...
              {/* Projektion */}
              <details>
                <summary className="cursor-pointer text-sm text-gray-700">Projektion (optional)</summary>
                <div className="grid grid-cols-1 gap-3 mt-2">
        ...
        {/* Ergebnis (alles einspaltig, nur gestapelte Karten) */}
        <section className="space-y-6">
          <div className="text-sm text-gray-700 font-medium flex items-center gap-2">
            Ergebnis <span className="text-xs text-gray-500">({viewTag})</span>
          </div>

          {/* Score Donut – Glow Card */}
          <div className="rounded-2xl p-[1px] bg-gradient-to-br from-emerald-300/70 via-sky-300/70 to-indigo-300/70">
          ...
          <div className="rounded-2xl border p-4 bg-white shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3 mt-4">
              ...
              <div className="rounded-xl border p-3">
                <div className="text-xs text-gray-500">Profitabel?</div>
                <div className="mt-1">
                  <span
                    className={
          ...
          {/* Wert vs. Kaufpreis – Glow, Gradients, Delta-Badge */}
          <div className="rounded-2xl p-[1px] bg-gradient-to-br from-indigo-300/70 via-emerald-300/70 to-sky-300/70">
            <div className="rounded-2xl bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between">
                <div>
              ...
              <p className="text-xs text-gray-600 mt-2">Hinweis: Cap Rate ist eine Annahme. Modell-Wert ist Orientierung – Lage/Zustand prüfen.</p>
            </div>
          </div>

          {/* Excel-ähnliche Auswertung */}
          <div className="rounded-2xl border p-4 bg-white shadow-sm space-y-6">
            <div>
              <div className="text-sm font-medium">Auswertung (Excel-Stil)</div>
            ...
            <div>
              <div className="text-sm font-medium mb-1">Monatsrechnung</div>
            ...
            <div className="grid grid-cols-1 gap-3">
              <InfoTile title="Vermögenszuwachs p.a. (ohne Wertsteigerung)" value={eur(Math.round(equityGainNoApp))} subtle />
              <InfoTile title="Vermögenszuwachs p.a. (mit Wertsteigerung)" value={eur(Math.round(equityGainWithApp))} subtle />
            </div>

            {/* 10J-Projektion – farbige Linien */}
            <div>
              ...
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={projection} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="vermoegenPA" name="Vermögenszuwachs p.a." stroke="#f59e0b" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              <p className="text-xs text-gray-600 mt-2">Vereinfachte Annuität: Zinsen auf Restschuld, Tilgung steigt, Annuität nominal konstant.</p>
            </div>
          </div>
          ...
          {/* Kurzeinschätzung */}
          <div className="rounded-2xl border p-4 bg-white shadow-sm">
            <div className="text-sm font-medium mb-2">Kurze Einschätzung <span className="text-xs text-gray-500">({viewTag})</span></div>
...
function InfoTile({ title, value, subtle }: { title: string; value: React.ReactNode; subtle?: boolean }) {
  return (
    <div className={`rounded-xl border p-3 ${subtle ? "bg-gray-50" : "bg-white shadow-sm"}`}>
...
function BreakEvenCard({ input, kaufpreis, mieteProM2Monat }: { input: WohnInput; kaufpreis:number; mieteProM2Monat:number }) {
  ...
  return (
    <div className="mt-4 rounded-xl border p-3 bg-gray-50">
      <div className="text-xs text-gray-500 mb-2">Break-even (CF = 0) bei heutigen Annahmen</div>
      <div className="grid gap-2 text-sm">
        <div className="flex items-center justify-between"><span>Kaufpreis-Grenze</span>
          <span className="tabular-nums">
            ...
            {bePrice != null && kaufpreis > 0 && (<span className="ml-2 text-xs text-gray-500">({signedPct((bePrice - kaufpreis) / kaufpreis)})</span>)}
          </span>
        </div>
        <div className="flex items-center justify-between"><span>benötigte Miete/m²</span>
          <span className="tabular-nums">
            {beRentPerM2.toFixed(2)} €/m²
            {mieteProM2Monat > 0 && (<span className="ml-2 text-xs text-gray-500">({signedPct((beRentPerM2 - mieteProM2Monat) / mieteProM2Monat)})</span>)}
          </span>
        </div>
      </div>
    </div>
