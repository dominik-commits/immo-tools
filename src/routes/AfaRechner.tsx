// src/routes/AfaRechner.tsx (v3.2 – playful UI, button + pie fixes)
...
const COLORS = {
  ...
  primaryAlt: "#60a5fa",  // blue-400
  accent: "#22c55e",      // green-500
  accentAlt: "#86efac",   // green-300
  warn: "#f59e0b",        // amber-500
  warnAlt: "#fde68a",     // amber-200
...
export default function AfaRechner() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold flex items-center gap-2">
            AfA-Rechner
            <span className="text-[11px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">v3.2</span>
          </h2>
          <p className="text-sm text-gray-600">Gebäudeanteil, Modernisierungen, Sonder-AfA – mit Pro-rata & einfacher Steuerwirkung.</p>
        </div>
      ...
      {/* Onboarding & Presets */}
      <div className="rounded-2xl border bg-gradient-to-br from-blue-50 to-emerald-50 p-4 space-y-3">
        <div className="flex items-center justify-between">
...
// ---- Buttons (sauber, keine doppelten Texte) ----
function Btn({
  label,
  onClick,
  variant = "primary",
...
function ModeToggle({ mode, setMode }: { mode: "basic" | "pro"; setMode: (m: "basic" | "pro") => void }) {
  return (
    <div className="inline-flex rounded-2xl border overflow-hidden bg-white shadow-sm">
      <button className={`px-3 py-1.5 text-xs ${mode === "basic" ? "bg-blue-50 text-blue-700" : ""}`} onClick={() => setMode("basic")} aria-pressed={mode === "basic"}>Einsteiger</button>
      <button className={`px-3 py-1.5 text-xs border-l ${mode === "pro" ? "bg-emerald-50 text-emerald-700" : ""}`} onClick={() => setMode("pro")} aria-pressed={mode === "pro"}>Pro</button>
    </div>
  );
}
...
function Help({ title }: { title: string }) {
  return (
    <span className="inline-flex items-center" title={title}>
      <svg viewBox="0 0 24 24" className="h-4 w-4 text-gray-400 ml-1" fill="none" stroke="currentColor" strokeWidth="2">
...
function NumberField({ label, value, onChange, step = 1, help }: { label: string; value: number; onChange: (v: number) => void; step?: number; help?: string }) {
  return (
    <label className="text-sm text-gray-700 block">
      ...
      <input className="mt-1 w-full border rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-blue-500/40" type="number" step={step} inputMode="numeric" value={Number.isFinite(value) ? value : 0} onChange={(e) => onChange(e.target.value === "" ? 0 : Number(e.target.value))} />
    </label>
  );
}
function PercentField({ label, value, onChange, step = 0.1, help }: { label: string; value: number; onChange: (v: number) => void; step?: number; help?: string }) {
  return (
    <label className="text-sm text-gray-700 block">
      <span className="inline-flex items-center">{label}{help && <Help title={help} />}</span>
      <input className="mt-1 w-full border rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/40" type="number" step={step} inputMode="decimal" value={Number.isFinite(value) ? value : 0} onChange={(e) => onChange(e.target.value === "" ? 0 : Number(e.target.value))} />
    </label>
  );
}
function TextField({ label, value, onChange, help }: { label: string; value: string; onChange: (v: string) => void; help?: string }) {
  return (
    <label className="text-sm text-gray-700 block">
      <span className="inline-flex items-center">{label}{help && <Help title={help} />}</span>
      <input className="mt-1 w-full border rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-emerald-500/40" type="text" value={value} onChange={(e) => onChange(e.target.value)} />
    </label>
  );
}
function SelectField<T extends string>({ label, value, options, onChange, help }: { label: string; value: T; options: { value: T; label: string }[]; onChange: (v: T) => void; help?: string }) {
  return (
    <label className="text-sm text-gray-700 block">
      <span className="inline-flex items-center">{label}{help && <Help title={help} />}</span>
      <select className="mt-1 w-full border rounded-xl p-2 bg-white focus:outline-none focus:ring-2 focus:ring-pink-500/40" value={value} onChange={(e) => onChange(e.target.value as T)}>
