...
// recharts removed - using tile-based design
...
function DetailsSection(props: {
  ...
  // 10J Projektion: letzter Wert für Trend-Anzeige
  ...
  return (
    <section style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 8 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {[
            ...
            <div key={row.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", background: "rgba(255,255,255,0.03)", borderRadius: 9 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: row.positive ? "#4ade80" : "#f87171", fontVariantNumeric: "tabular-nums" }}>
                {row.positive ? "+" : ""}{eur(row.value)}
              </span>
            </div>
          ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>Betriebsergebnis (NOI)</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: "#FCDC45", fontVariantNumeric: "tabular-nums" }}>{eur(Math.round(noi))}</span>
              </div>
              <div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${Math.min(100, Math.round(noi / Math.max(noi, annuitaetJahr) * 100))}%`, background: "#FCDC45", borderRadius: 3 }} />
              </div>
            </div>
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>Kreditrate p.a.</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: "#7c3aed", fontVariantNumeric: "tabular-nums" }}>{eur(Math.round(annuitaetJahr))}</span>
              </div>
              <div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${Math.min(100, Math.round(annuitaetJahr / Math.max(noi, annuitaetJahr) * 100))}%`, background: "#7c3aed", borderRadius: 3 }} />
              </div>
            </div>
            <div style={{ marginTop: 6, padding: "8px 12px", background: noi >= annuitaetJahr ? "rgba(74,222,128,0.07)" : "rgba(248,113,113,0.07)", borderRadius: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      {/* ── 10-Jahres Projektion als Kacheln ───────────────── */}
      <div style={C.card}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", marginBottom: 16 }}>10-Jahres-Projektion (Hochrechnung)</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          <div style={{ padding: "14px", background: "rgba(255,255,255,0.03)", borderRadius: 10, textAlign: "center" }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginBottom: 6 }}>Betriebsergebnis Jahr 10</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#FCDC45", fontVariantNumeric: "tabular-nums" }}>{lastProj ? eur(Math.round(lastProj.noi)) : "–"}</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>p.a.</div>
          </div>
          <div style={{ padding: "14px", background: "rgba(255,255,255,0.03)", borderRadius: 10, textAlign: "center" }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginBottom: 6 }}>Cashflow Jahr 10</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: lastProj && lastProj.cf >= 0 ? "#4ade80" : "#f87171", fontVariantNumeric: "tabular-nums" }}>{lastProj ? eur(Math.round(lastProj.cf)) : "–"}</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>p.a.</div>
          </div>
          <div style={{ padding: "14px", background: "rgba(255,255,255,0.03)", borderRadius: 10, textAlign: "center" }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginBottom: 6 }}>CF-Entwicklung</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: cfTrend >= 0 ? "#4ade80" : "#f87171", fontVariantNumeric: "tabular-nums" }}>{cfTrend >= 0 ? "+" : ""}{eur(Math.round(cfTrend))}</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>über 10 Jahre</div>
          </div>
        </div>
        <div style={{ marginTop: 12, padding: "8px 12px", background: "rgba(255,255,255,0.02)", borderRadius: 8, fontSize: 11, color: "rgba(255,255,255,0.3)", lineHeight: 1.6 }}>
          Hochrechnung basierend auf deinen Miet- und Kostensteigerungsannahmen. Leerstand, Annuität und Zinssatz bleiben konstant. Kein Ersatz für individuelle Finanzplanung.
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <div style={C.card}>
          ...
          <div style={{ marginTop: 10, fontSize: 11, color: "rgba(255,255,255,0.28)", lineHeight: 1.5 }}>
            Konstante Annuität, gleichbleibender Zinssatz. Ohne Sondertilgungen.
          </div>
        </div>
      </div>
