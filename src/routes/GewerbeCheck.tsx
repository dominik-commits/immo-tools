 ...
 * - Mieter-Incentives je Zone: Free-Rent (Y1, Monate) & TI (€/m², upfront)
 * - Opex gesamt (% auf Brutto); Recoverables entlasten Vermieter
...
type Zone = {
  ...
  leaseTermYears: number;     // Restlaufzeit/WALT dieser Zone (für Info/Spread)
};
...
function Badge({ icon, text, hint }: { icon: React.ReactNode; text: string; hint?: string }) {
  return (<span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-[11px] text-gray-700 bg-white shadow-sm" title={hint}>{icon} {text}</span>);
}
...
function NumberField({ label, value, onChange, step = 1, help }: { label: string; value: number; onChange: (n:number)=>void; step?: number; help?: string }) {
  return (
    <div>
      ...
      <input className="mt-1 w-full border rounded-lg p-2" type="number" step={step} value={Number.isFinite(value) ? value : 0} onChange={(e)=>onChange(e.target.value === "" ? 0 : Number(e.target.value))}/>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="w-28 text-right tabular-nums">{pct(value)}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <input type="range" min={0} max={0.95} step={step} value={value} onChange={(e)=>onChange(Number(e.target.value))} className="w-full" />
        <input type="number" step={0.1} className="w-16 border rounded-lg p-1 text-right" value={(value*100).toFixed(1)} onChange={(e)=>onChange(Number(e.target.value)/100)} />
...
export default function GewerbeCheck() {
  ...
  const [opexTotalPctBrutto, setOpexTotalPctBrutto] = useState(0.26); // gesamt Opex (recoverable + non-recoverable)
  ...
  const tiUpfront       = zonenCalcY1.totalTI; // Einmalzahlung t0 (Liquide Mittelabfluss)
  ...
  const annuityYear = financingOn ? annuityExact(loan, zinsPct, laufzeitYears) : 0; // exakte jährliche Rate
  const interestY1  = financingOn ? loan * zinsPct : 0; // Näherung Y1 (genau im Plan)
  const principalY1 = financingOn ? Math.max(0, annuityYear - interestY1) : 0;

  const cashflowMonatY1 = (noiY1 - annuityYear - tiUpfront) / 12; // TI einmalig in Y1 abgezogen
  ...
  // Projektion (10y – Free-Rent nur in Y1, TI nur t0; Mieten wachsen, Opex/CapEx wachsen)
    ...
    <div className="bg-gradient-to-b from-gray-50 to-white min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6 pb-40">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-slate-700 to-gray-900 text-white grid place-items-center shadow">
              <Briefcase className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-xl font-semibold tracking-tight">Gewerbe-Check</h2>
              <p className="text-gray-600 text-sm">Exakte Annuität, Recoverables je Zone & Incentives – übersichtlich wie MFH.</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        <section className="space-y-4">
          ...
          <Card>
            <div className="flex items-center justify-between">
              <div className="text-sm font-medium">Mietzonen</div>
              <button
                className="text-xs inline-flex items-center gap-1 px-2 py-1 rounded-lg border bg-white hover:bg-gray-50"
              ...
              {zonen.map((z, idx) => (
                <div key={z.id} className="grid grid-cols-12 gap-2 items-end">
                  <div className="col-span-3">
                    ...
                    <input className="mt-1 w-full border rounded-lg p-2" value={z.name} onChange={(e)=>updateZone(z.id, { name: e.target.value })}/>
                  </div>
                  <div className="col-span-2">
                    <LabelWithHelp label="Fläche (m²)" />
                    <input type="number" className="mt-1 w-full border rounded-lg p-2" value={z.areaM2} onChange={(e)=>updateZone(z.id, { areaM2: num(e.target.value, z.areaM2) })}/>
                  </div>
                  <div className="col-span-2">
                    <LabelWithHelp label="Miete (€/m²/Monat)" />
                    <input type="number" step={0.1} className="mt-1 w-full border rounded-lg p-2" value={z.rentPerM2} onChange={(e)=>updateZone(z.id, { rentPerM2: num(e.target.value, z.rentPerM2) })}/>
                  </div>
                  <div className="col-span-2">
                  ...
                  <div className="col-span-12 text-xs text-gray-600 -mt-1">
            ...
            <div className="grid grid-cols-1 gap-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select className="mt-1 w-full border rounded-lg p-2" value={bonitaetTop3} onChange={(e)=>setBonitaetTop3(e.target.value as Bonitaet)}>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="rounded-lg border p-3 bg-white">
                  ...
                  <div className="mt-1 text-lg font-semibold tabular-nums">
                    {pct(capEff)} <span className="text-xs text-gray-500">({signedPct(capSpread)} Spread)</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <Card className="bg-gray-50">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">Exakte Annuität: Rate = L·r / (1−(1+r)⁻ⁿ)</span>
            </div>
            {financingOn && (
          ...
          <Card>
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-gray-600 mb-3">Kaufpreis & Ø-Miete gemeinsam justieren.</p>
              </div>
        ...
        <section className="space-y-6">
          ...
          <Card className="shadow-sm">
            <div className="text-sm font-medium mb-2">Kostenaufteilung (Y1, auf Brutto)</div>
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={[
                    ...
                    <Cell fill="#93c5fd" /><Cell fill="#60a5fa" /><Cell fill="#fca5a5" /><Cell fill="#fcd34d" />
                  </Pie>
            ...
            <p className="text-xs text-gray-600 mt-2">Recoverables entlasten den Vermieter; TI ist Einmalzahlung (separat in CF).</p>
          </Card>
          ...
          <Card className="shadow-sm">
            ...
            <p className="text-xs text-gray-600 mt-2">Y1: Free-Rent & TI wirken einmalig; ab Y2 normalisiert sich die Miete (mit Growth).</p>
          </Card>
          ...
          {/* NK-Detail */}
          <Card className="shadow-sm">
            <div className="text-sm font-medium mb-2">Kaufnebenkosten im Detail</div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20">
        <div className="mx-auto max-w-3xl px-4 pb-[env(safe-area-inset-bottom)]">
          <div className="mb-3 rounded-2xl border shadow-lg bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/60">
            ...
            {/* Progress-Bar als spielerisches UI-Element */}
...
function ValueVsPrice({ KP, wertAusCap, valueGap, valueGapPct, capEff, capRateAssumed, capSpread, viewTag }:{
  ...
  return (
    <div>
      <div className="flex items-start justify-between">
        <div>
          <div className="text-sm font-medium">Wert (NOI/Cap_eff) vs. Kaufpreis</div>
      ...
      <p className="text-xs text-gray-600 mt-2">Effektive Cap: {pct(capEff)} (Basis {pct(capRateAssumed)} {capSpread>=0?"+":"-"} {Math.abs(capSpread*100).toFixed(1)} bp Risiko).</p>
    </div>
  );
}
...
function AmortTable({ plan }:{ plan:{ rows:{year:number; interest:number; principal:number; annuity:number; outstanding:number;}[]; sum10:{interest:number; principal:number; annuity:number;} } }){
  ...
  return (
    <Card className="shadow-sm">
      <div className="text-sm font-medium mb-2">Tilgungsplan (exakte Annuität)</div>
      ...
      <p className="text-xs text-gray-600 mt-2">TI wird als einmaliger Abfluss in Y1 berücksichtigt (nicht in der Annuität).</p>
    </Card>
  );
}
...
function computeZonesY1(zonen:Zone[], rentAdjPct:number){
  ...
  const recoveredOpexFromZones:( (opexPct:number)=>number ) = (opexPct:number)=>{
    // sum over zones: gross_z * opexPct * recoverablePct_z
...
function buildProjection10y(opts:{
  ...
  for (let t=1; t<=years; t++){
    ...
    for (const z of zones){
      const freeFactorY1 = (t===1) ? (1 - Math.min(z.freeRentMonthsY1,12)/12) : 1; // nur Y1
...
function calcCapSpread(walt:number, bonitaet:Bonitaet, indexiert:boolean){
  let spreadWALT = walt < 3 ? 0.006 : walt < 5 ? 0.003 : walt < 8 ? 0.000 : -0.002;
  let spreadBon  = bonitaet === "A" ? -0.002 : bonitaet === "B" ? 0.000 : 0.004;
  let spreadIdx  = indexiert ? -0.001 : 0;
