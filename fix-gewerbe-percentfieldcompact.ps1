# fix-gewerbe-percentfieldcompact.ps1
# Behebt den Mehrstellig-Eingabe-Bug im PercentFieldCompact von GewerbeCheck.tsx
# Ausfuehren aus dem Projektroot (frontend-Ordner):
#   powershell -ExecutionPolicy Bypass -File .\fix-gewerbe-percentfieldcompact.ps1
...
if ($normalized.Contains($oldBlock)) {
    Copy-Item $path "$path.bak-percentcompactfix" -Force
    Write-Host "Backup angelegt: $path.bak-percentcompactfix"
    $updated = $normalized.Replace($oldBlock, $newBlock)
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    [System.IO.File]::WriteAllText((Resolve-Path $path), $updated, $utf8NoBom)
    Write-Host "[OK] $path (PercentFieldCompact) gepatcht"
} else {
